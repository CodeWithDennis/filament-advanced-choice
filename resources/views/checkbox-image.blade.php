            ...
            @forelse ($options as $value => $label)
                ...
                >
                    <label
                        for="{{ $id }}"
                        @class([
                            'fi-fo-checkbox-list-option group relative flex flex-col overflow-hidden rounded-lg border border-gray-300 bg-white dark:border-gray-700 dark:bg-gray-900 has-checked:outline-2 has-checked:-outline-offset-1 has-checked:outline-custom-600 dark:has-checked:outline-custom-500 has-disabled:opacity-60 has-disabled:cursor-not-allowed',
                    ...
                    >
                        ...
                        <div class="fi-fo-checkbox-list-option-text flex flex-1 flex-col p-4">
                            <span class="fi-fo-checkbox-list-option-label block text-sm font-medium text-gray-900 dark:text-gray-100">
                                @if ($isHtmlAllowed)
                                    {!! $label !!}
                                @else
                                    {{ $label }}
                            ...
                            @if ($description)
                                <span class="fi-fo-checkbox-list-option-description mt-1 block text-sm text-gray-500 dark:text-gray-400">
                                    {{ $description }}
                                </span>
                            @endif
                            @if ($extra)
                                <span class="fi-fo-checkbox-list-option-extra mt-3 block text-sm font-medium text-gray-900 dark:text-gray-100">
                                    {{ $extra }}
                                </span>
                            @endif
                        </div>
                            ...
                            <input
                                ...
                                {{
                                    $extraInputAttributeBag
                                        ...
                                        ->class([
                                            'fi-checkbox-input absolute end-3 top-3 size-4 shrink-0 checked:bg-custom-500 checked:border-custom-500 hover:checked:bg-custom-600 hover:checked:border-custom-600 focus:border-custom-500 focus:ring-custom-500',
                        ...
                        @if ($hiddenInputs)
                            <x-filament::icon
                                :icon="$getHiddenInputIcon()"
                                class="invisible absolute end-2 top-2 size-5 text-custom-600 group-has-checked:visible dark:text-custom-500"
                            />
                        @endif
                    </label>
                </div>
